...
FILENAME="rootfs/usr/share/${filename}.xml"
...
read -p "Enter minimal allowed temperature [${MIN_TEMPERATURE}]: " min_temp
read -p "Enter maximal allowed temperature [${MAX_TEMPERATURE}]: " max_temp
...
for fanMode in ${FAN_MODES[*]}; do
    for tempMode in ${TEMPERATURE_MODES[*]}; do
...
for swingMode in ${SWING_MODES[*]}; do
